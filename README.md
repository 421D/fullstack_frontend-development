# Fullstack Frontend Development Projects

This repository consolidates a collection of **full-stack and frontend-focused projects** spanning **mobile, desktop, web, and mini program ecosystems**.  
It demonstrates practical experience across **UI/UX design, frontend engineering, backend integration, local/cloud data persistence**, and **end-to-end application workflows**.

The projects range from **early-stage frontend practice** to **complete application systems**, covering Android, Java desktop systems, Web platforms, and WeChat Mini Programs.

---

## 1. Tech Stack Overview

### Frontend
- HTML5 / CSS3 / JavaScript (ES6+)
- jQuery
- Android UI (XML, RecyclerView)
- Java Swing
- WeChat Mini Program (WXML, WXSS)
- Figma / Pixso (UI & UCD prototyping)

### Backend & Data
- Java (JDBC)
- PHP (Dedecms CMS)
- Local JSON storage
- SQL Server
- Tencent Cloud Database
- WeChat Cloud Functions (Serverless)

### Tools & Platforms
- Android Studio
- Eclipse
- WeChat Developer Tools
- Gradle
- Git / GitHub



## 2. Projects Index

### ① Android Projects
#### [Daily News App（今日理财资讯）](./Android_Projects)

An Android finance management application built with **Java** in **Android Studio**, focusing on structured data display and local persistence.

**Highlights**
- Splash screen with animation / MP4 intro
- RecyclerView-based dynamic list
- Add & manage finance items in real time
- Local JSON storage for offline persistence

**Key Tech:** Java, Android SDK, RecyclerView, JSON, MediaPlayer



### ② Java Desktop Systems
#### [Hotel Management System（酒店管理系统）(./Java_projects)

A **full-featured desktop management system** using **Java Swing + SQL Server**, supporting authentication, room management, billing, and role-based access.

**Highlights**
- Swing-based GUI with modular OOP architecture
- JDBC-driven database operations
- Separate the authentication server and the main client
- Complete hotel workflow: check-in, dining, billing, checkout

**Key Tech:** Java, Swing, JDBC, SQL Server



### ③ WeChat Mini Program Projects
#### [WeChat Mini Program Projects Collection（微信小程序集）](./WeChatMiniProgram_Projects)

A set of Mini Programs showcasing **frontend engineering + cloud backend integration** within the WeChat ecosystem.

| Project | Focus | Tech |
|-------|------|------|
|Community_WeChat_MiniProgram | Community & property management | Cloud DB, Cloud Functions |
|PM25Studio_WeChat_MiniProgram | Media & creative platform | WXML, WXSS, JS |
|MediaHub_WeChat_MiniProgram | Multimedia & system tools | WeChat APIs |

**Key Tech:** WeChat Cloud Development, Tencent Cloud DB, WeChat Native APIs



### ④ Web Projects
#### [Web Projects Collection](./Web_Projects)

A progression from **static frontend pages** to **CMS-driven dynamic websites** and **professional UI/UX prototypes**.

**Included Projects**
- Mobile E-Commerce Mockup (HTML/CSS/JS)
- YiCha Static & Dynamic Tea Website (jQuery + Dedecms)
- Cosmetic_Store_Mockup

**Key Tech:** HTML, CSS, JS, jQuery, PHP, CMS, Figma



## 3. Skills Demonstrated

- Frontend architecture & UI component design
- Full CRUD workflows across platforms
- Backend integration (local + cloud)
- Database schema understanding
- Serverless backend logic
- User-centred design (UCD) & accessibility
- Cross-platform development mindset


## 4. Positioning Note

This repository reflects **hands-on full-stack capability** rather than isolated demos:
- Mobile (Android)
- Desktop (Java Swing)
- Web (Static + CMS)
- Mini Programs (Cloud-native)

Each project focuses on **real application structure**, **data flow**, and **user interaction**, aligned with industry expectations for junior–mid full-stack or frontend engineers.




## 5. Repository Structure

```
fullstack_frontend-development/
│
├── Android_Projects/
│ └── Daily News App (Android / Java)
│
├── Java_projects/
│ └── Hotel Management System (Java Swing + SQL Server)
│
├── WeChatMiniProgram_Projects/
│ ├── Community_WeChat_MiniProgram
│ ├── PM25Studio_WeChat_MiniProgram
│ └── MediaHub_WeChat_MiniProgram
│
└── Web_Projects/
├── Mobile_ECommerce_Mockup
├── YiCha_Static
├── YiCha_Dynamic_DedeCMS
└── Cosmetic_Store_Mockup
```


